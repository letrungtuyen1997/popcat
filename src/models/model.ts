...
export class FBStorage implements Storable {
	...
	commit() {
		...
		FBInstant.player.setDataAsync(saveObj).then(() => {
			...
			return G.ldb.setScoreAsync((<any>this)['score']).catch(error => console.log(error))
			/*.then(leaderboard => {
	...
	getLeaderBoard(entriesNo:number, offset:number) : Promise<FBInstant.LeaderboardEntry[]>{
		return new Promise<FBInstant.LeaderboardEntry[]>((resolve, reject) => {
			/*FBInstant
			.getLeaderboardAsync(this.leaderBoardName)
			.then(leaderboard => {
				leaderboard.getEntriesAsync(entriesNo, 0).then( entries => {
					resolve(entries)
				}).catch( e => console.log(e))
			}).catch( e => console.log(e))*/		
			G.ldb.getEntriesAsync(entriesNo, 0).then( entries => {
				resolve(entries)
			}).catch( e => console.log(e))
		})
	}
